    ...
    public partial class VeryAnimation
    {
        ...
        private void ToolsKeyframeReduction(AnimationClip clip)
        {
            ...
            try
            {
                ...
                transformPoseSave.ResetDefaultTransform();
                ...
                if (result)
                {
                    ...
                    {
                        var p = exporter.exportedFiles[0];
                        ...
                        if (importer is ModelImporter)
                        {
                            ...
                            if (isHuman)
                            {
                                modelImporter.animationType = ModelImporterAnimationType.Human;
                                {
                                    ...
                                    {
                                        hd.upperArmTwist = uAvatar.GetUpperArmTwist(editAnimator.avatar);
                                        hd.lowerArmTwist = uAvatar.GetLowerArmTwist(editAnimator.avatar);
                                        hd.upperLegTwist = uAvatar.GetUpperLegTwist(editAnimator.avatar);
                                        hd.lowerLegTwist = uAvatar.GetLowerLegTwist(editAnimator.avatar);
                                        hd.armStretch = uAvatar.GetArmStretch(editAnimator.avatar);
                                        hd.legStretch = uAvatar.GetLegStretch(editAnimator.avatar);
                                        hd.feetSpacing = uAvatar.GetFeetSpacing(editAnimator.avatar);
                                        hd.hasTranslationDoF = uAvatar.GetHasTDoF(editAnimator.avatar);
                                        {
                                            var humanBones = new List<HumanBone>();
                                            for (int i = 0; i < humanoidBones.Length; i++)
                                            {
                                                if (humanoidBones[i] == null)
                                                    continue;
                                                humanBones.Add(new HumanBone()
                                                {
                                                    boneName = humanoidBones[i].name,
                                                    humanName = HumanTrait.BoneName[i],
                                                    limit = new HumanLimit()
                                                    {
                                                        useDefaultValues = false,
                                                        min = humanoidMuscleLimit[i].min,
                                                        max = humanoidMuscleLimit[i].max,
                                                        center = uAvatar.GetLimitSign(editAnimator.avatar, i),
                                                        axisLength = uAvatar.GetAxisLength(editAnimator.avatar, i),
                                                    },
                                                });
                                            }
                                        ...
                                        {
                                            var tqs = uAvatar.GetDefaultPoseTQS(editAnimator.avatar);
                                            var skeletonBones = new List<SkeletonBone>(tqs.Length);
                                            for (int i = 0; i < tqs.Length; i++)
                                            {
                                                skeletonBones.Add(new SkeletonBone()
                                                {
                                                    name = Path.GetFileNameWithoutExtension(tqs[i].path),
                                                    position = tqs[i].position,
                                                    rotation = tqs[i].rotation,
                                                    scale = tqs[i].scale,
                                                });
                                            }
                    ...
                    {
                        var p = exporter.exportedFiles[1];
                        var pTmp = p.Replace(Application.dataPath, "Assets");
                        AssetDatabase.ImportAsset(pTmp);
                        var importer = AssetImporter.GetAtPath(pTmp);
                        if (importer is ModelImporter)
